...
**安全兜底**：`alarm` 超 60 秒无人处理自动回落；空闲 5 分钟自动全灭。
...
| 兜底 | 默认值 | 作用 |
|---|---|---|
| `alarm` 超时回落 | **60 秒** | 你关掉弹窗后没人处理，灯自动回到之前的状态 |
| 空闲自动全灭 | **5 分钟** | 电脑异常退出/DSH 崩溃后，灯不会一直亮着 |

两个值都能在 `~/.dsh/profiles/web/cordis.patch.yml` 里改：

```yaml
- id: dsh-led-bridge
  config:
    alarmTimeoutMs: 30000   # 改成 30 秒
    idleTimeoutMs: 0        # 0 = 永不自动全灭
```
