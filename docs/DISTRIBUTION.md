...
### v0.2.7：**修复只做了一半**，而且我的测试两次给出假结论

v0.2.6 修了"notify 闪烁期间到达的状态被回滚"。逻辑上对，模拟器也过，
**但真机上红灯依然不亮**。这一轮把它查清了，过程本身比 bug 更值得记。

#### 真正的 bug：清空快照之后，兜底把状态打成了全灭

v0.2.6 只做了两件事中的第一件：

| 步骤 | v0.2.6 | 后果 |
|---|---|---|
| 状态真的改变时**作废快照** | ✅ 做了 | 阻止了"旧状态覆盖新状态" |
| 闪烁结束时**无目标则不调 applyCommand** | ❌ 没做 | 仍走 `target = "off"` 兜底 → **新状态被打成全灭** |

我当时在注释里写「`applyCommand("off")` 恰好等于当前状态、无害」——**那句是错的**。
它只在 `currentState` 本来就是 `off` 时成立。真机实测：

```
error 回执：["OK","error","OK notify -> off"]
闪完：state=off  saved=<none>  savedcleared=1
                              ↑ 修复路径确实执行了
                    ↑ 但状态被打成了 off
```

**两处缺一不可。** 现在结束分支改成：
`if (target.length() > 0) applyCommand(target); else 保持现状`。

#### 我的测试两次给出假结论

**假阳性**：早期版本的 C2 用例在 `notify` 与 `error` **之间插了一次 `state?` 查询**。
查询要几百毫秒到几秒（还要重试拆包），而闪烁窗口只有 600ms ——
`error` 到达时闪烁**早已结束**，用例根本没撞上竞态，却因为"最终状态是 error"判为通过。

**不稳定**：改成不插查询后，间隔取了 300ms —— 正好卡在 600ms 窗口边界，
同一份固件两次跑出不同结果。

**戳穿它们的是计数器**：`savedcleared`（快照作废累计次数）不受查询时机影响。
第一次跑出 `0 → 0`，直接暴露"没撞上窗口"；修好时序后 `0 → 1`，才证明真的测到了。

#### 顺带修的一串工具缺陷

这一轮排查里有相当一部分时间耗在**我自己的工具**上，不是固件：

1. **`state?` 解析器**假设"行/词/字段"边界稳定 —— 实际 USB CDC 的拆包位置完全随机：
   见过行首拆开、**单词中间拆开**（`"STATE think"` + `"ing | plan=..."`）、
   **字段中间拆开**（`"STATE error | plan="` + `"0 tools=..."`）。
   这些全被判成"固件没响应 state?" —— **把我引向了完全错误的方向**。
   改成把收到的文本拼成一条流、直接匹配两个已知格式的字段。
2. **查询每次尝试都清空缓冲** → 慢到跨两次尝试的响应被丢掉。改成跨尝试累积。
3. **丢弃 `send()` 的回执** → "命令没被接收"和"命令被接收但没生效"长得一模一样。
   现在每步都打印原始回执。
4. **序列化脚本路径依赖**：反复用绝对路径去引另一个工程的 `node_modules`。
   抽成 `scripts/_serial.mjs`，按 环境变量 → repo/plugin → dist 产物 → cwd 顺序解析。
5. **审计断言匹配到自己的注释**：注释里刻意引用了旧代码当反面教材
   （`if (target.length()==0) target = "off"`），全文正则会误伤。
   改成只查去掉注释后的源码 —— 这类"注释里放反例"的做法要配套这个处理。

> **教训**：这一轮里，**我的观测工具比被观测的对象更不可靠**。
> 一个读错的传感器会让人往完全错误的方向修。所以"验证工具本身要先被验证" ——
> 这也是为什么后来加了 `savedcleared` 计数器：它把"时序敏感的观察"换成了"累计事实"。
