...
> **没有「空闲自动灭灯」的配置项，这是故意的。**
> 它曾经有（`idleTimeoutMs`），已删除 —— 因为固件里本来就有一份无条件的
> `STALE_TIMEOUT_MS`，两者是同一件事的两份实现：
>
> 1. **那是个骗人的旋钮**：设成 `0`（本意"永不自动灭"）或任何 ≥300000 的值都**不生效**，
>    固件照样在 5 分钟时把灯全灭。
> 2. **两者语义不同，会互相打架**：插件那份测「多久没有**新状态**」
>    （只在状态/工具上报时重新武装），固件那份测「多久没有**串口活动**」
>    （每条被接受的命令都重新武装）。一次很长的推理（例如压缩上下文，
>    `thinking` 持续几分钟，中间只有不发命令的 `step/start`）会先触发插件那个定时器，
>    把**正在工作**的灯灭掉。
>
> 现在只剩固件那一份 —— 它更安全，而且是真正的兜底（电脑崩了板子自己会灭灯）。
> **要改这个超时**：改 `esp32c3_dsh_status_light.ino` 里的 `STALE_TIMEOUT_MS` 并重烧。
